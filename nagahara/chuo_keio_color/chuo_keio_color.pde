...
void setup(){
  size(1416, 1000);
//  size(1416, 1000, PDF, "chuo_keio.pdf");
...
void draw(){
  ...
  for(int i = 0; i<memes.size(); i++){
    ...
    float diameter = sqrt(map(meme.passengers, 0, 1000000, 0, 33333));
    float x = cos(radians(meme.radian-90)) * meme.time;
    float y = sin(radians(meme.radian-90)) * meme.time;
    float c = map(meme.difference, 0.7, 1.3, 210, 360);
    x = x * 20;
    y = y * 20;
